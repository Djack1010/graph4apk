...
public class PDGToDotGraph {
   ...
   */
  public PDGToDotGraph(ProgramDependenceGraph pdg, String name) {
    this.pdg = pdg;
    this.name = name;
  ...
  private void recursiveDrawPDG(PDGNode pdgNode) {
    ...
    if (pdgNode.getDependents().isEmpty() && pdgNode.getBackDependets().isEmpty()) {
      return;
    } else {
      Set<PDGNode> nextVisits = new HashSet<PDGNode>();
      if (!pdgNode.getDependents().isEmpty()) {
        Iterator<PDGNode> iteraNodes = pdgNode.getDependents().iterator();
