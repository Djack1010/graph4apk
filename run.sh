...
TEST=/home/giacomo/Documents/merc_proj/apk_test/0d4a16a36a62e4d9bc6e466729a55094.apk

APKDB="/home/giacomo/Documents/merc_proj/apk_db"
...
#exit
for apkfile in $APKDB/*.apk; do
    filename=$(echo ${apkfile##*/} | cut -d'.' -f 1)
    #echo $filename
    start=$(date +%s)
    echo "-----  ANALYZING $apkfile  -----"
    eval "$RUN -process-dir $apkfile -SDGFileName $filename"
    end=$(date +%s)
    runtime=$(($end-$start))
